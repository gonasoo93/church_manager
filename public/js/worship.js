...
function renderWorship(worship) {
    ...
    <tr>
      ...
      <td><strong>${w.sermon_title || '-'}</strong></td>
      <td>${w.preacher || '-'}</td>
      <td>${w.sermon_text || '-'}</td>
...
function showWorshipForm(worship = null) {
    ...
    <form id="worship-form">
      ...
      <div class="form-group">
        ...
        <input type="text" id="worship-sermon-title" value="${worship?.sermon_title || ''}">
      </div>
      <div class="form-group">
        <label for="worship-sermon-text">본문 말씀</label>
        <input type="text" id="worship-sermon-text" placeholder="예: 요한복음 3:16" value="${worship?.sermon_text || ''}">
      </div>
      <div class="form-group">
  ...
  `;
    ...
    modal.querySelector('[data-action="submit"]').addEventListener('click', async () => {
        const formData = {
            ...
            sermon_title: document.getElementById('worship-sermon-title').value,
            sermon_text: document.getElementById('worship-sermon-text').value,
...
async function viewWorship(id) {
    try {
      ...
      <div style="display: flex; flex-direction: column; gap: 1rem;">
        ...
        <div><strong>설교 제목:</strong> ${worship.sermon_title || '-'}</div>
        <div><strong>본문 말씀:</strong> ${worship.sermon_text || '-'}</div>
